...
dir_res, mode, eqn_type, scheme_name, FD, RK, Nelem, CFL, dt_, sol_dir, exact_sol_dir, T_period_ = FD_input_reader(args.python_input)

t_plot = Decimal(args.time_for_plot)
t_plot =  Decimal(t_plot.quantize(Decimal('.001')))

if eqn_type=='linear_advec':
    temp_sol_plot(dir_res, mode, scheme_name, FD, RK, Nelem, CFL, dt_, t_plot, sol_dir, exact_sol_dir,T_period_)
